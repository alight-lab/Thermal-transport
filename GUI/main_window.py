...
class MainWindow(QWidget, Ui_Form):
    ...
    def start_main(self):
        ...
        # 建立晶体数据文件
        self.x = lattice_set(element1, element2, element3, size1, size2, size3)
        ...
        self.t = Work(element1, element2, element3, filepath, temperature0, temperature_set,
                        ...
                        iterate_time_heat, defect_num, self.x)
        self.t.start()
    ...
    def plot(self):
        ...
        with open('result_iterate.txt', 'r') as f:
            ...
            while True:
                line = f.readline()
                data = line.split()
                iterate_x.append(float(data[0]))
                iterate_y.append(float(data[1]))
                if f.tell() >= eof:  #将当前位置与文件尾(eof)比较
                    break
        if len(iterate_x) != 0:
            cav = plot_2d(np.array(iterate_x) * 1e-3, np.array(iterate_y), 'Time/ps', 'Temperature/K', 'Temperature')
