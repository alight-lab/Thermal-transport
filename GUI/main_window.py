...
class MainWindow(QWidget, Ui_Form):
    ...
    def plot(self):
        ...
        if len(self.t.Time_axis) != 1:
            cav = plot_2d(np.array(self.t.Time_axis) * 1e12, self.t.T_axis, 'Time/ps', 'Temperature/K', 'Temperature')
            self.verticalLayout_2.addWidget(cav)
            cav = plot_2d(np.array(self.t.Time_axis) * 1e12, self.t.T_axis, 'Time/ps', 'Temperature/K', 'Temperature')
            self.verticalLayout_6.addWidget(cav)
        if len(self.t.x_chunk) != 0:     # 保证暂停时不绘图
            cav = plot_2d(np.array(self.t.x_chunk) * 1e10, self.t.T_chunk, 'X/Å', 'Temperature/K', 'Temperature')
